...
local function SendChopshopDispatch()
    ...
    TriggerServerEvent('cd_dispatch:AddNotification', {
        ...
        message = ('A %s dismantling a vehicle at %s'):format(data.sex, data.street),
        flash = 0,
        unique_id = data.unique_id,
        sound = 1,
        blip = {
            sprite = 595, scale = 1.2, colour = 1, flashes = true,
            text = '911 - Vehicle Chopping', time = 5, radius = 0
        }
    })
end
...
local function StartChoppingPart(vehicle, partIndex, partLabel)
    if (GetGameTimer() - lastChopAttempt) < CHOP_ACTION_COOLDOWN then
        lib.notify({
            title = 'Chopshop',
            description = 'You need to wait a moment',
            type = 'error'
        })
        return
    end
    ...
    if not isInChopZone then
        lib.notify({
            title = 'Chopshop',
            description = 'You must be in the chopshop area!',
            type = 'error'
        })
        return
    end
    ...
    if #(playerCoords - vehicleCoords) > 10.0 then
        lib.notify({
            title = 'Chopshop',
            description = 'Too far from vehicle!',
            type = 'error'
        })
        return
    end
    ...
    if not activeVehicles[vehicle] or (vehicleStateBag.partsRemoved and vehicleStateBag.partsRemoved[partIndex]) then
        lib.notify({
            title = 'Chopshop',
            description = 'This part has already been removed!',
            type = 'error'
        })
        return
    end
    ...
    local success = lib.progressCircle({
        duration = Config.chopshop.chopTime,
        position = 'bottom',
        label = ('Dismantling %s with circular saw...'):format(partLabel),
        canCancel = true,
    ...
    if success then
        if not DoesEntityExist(vehicle) then
            lib.notify({
                title = 'Chopshop',
                description = 'Vehicle disappeared during dismantling!',
                type = 'error'
            })
            return
        end
        ...
        if AreAllPartsRemoved(activeVehicles[vehicle].partsRemoved) then
            ...
            lib.notify({
                title = 'Chopshop',
                description = 'Vehicle completely dismantled! It will be disposed of shortly.',
                type = 'success'
            })
        ...
        lib.notify({
            title = 'Chopshop',
            description = 'Dismantling cancelled.',
            type = 'error'
        })
    end
end
        ...
        if not partsRemovedLocal and not partsRemovedState then
            ...
            if boneIndex ~= -1 then
                table.insert(options, {
                    ...
                    icon = 'fas fa-cut',
                    label = 'Dismantle ' .. partConfig.label,
                    bones = {partConfig.bone},
                    distance = 2.5,
                    onSelect = function()
                        StartChoppingPart(vehicle, partIndex, partConfig.label)
                    end,
...
CreateThread(function()
    while true do
        ...
        local wasInZone = isInChopZone
        isInChopZone = dist <= Config.chopshop.zone.radius
        
        if isInChopZone and not wasInZone then
            lib.notify({
                title = 'Illegal Chopshop',
                description = 'You\'ve entered the chopshop area. Bring stolen vehicles here to dismantle them.',
                type = 'inform'
            })
        elseif not isInChopZone and wasInZone then
            lib.notify({
                title = 'Chopshop',
                description = 'You\'ve left the chopshop area.',
                type = 'inform'
            })
        end
    end
